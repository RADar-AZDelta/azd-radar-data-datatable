...
export default class DataTable extends DataTableConfig {
    ...
    replaceValuesOfColumn = async (currentValue: any, updatedValue: any, column: string) => await data.dataTypeImpl?.replaceValuesOfColumn(currentValue, updatedValue, column)
    ...
    async getNextRow(currentIndex: number) {
        const { rowsPerPage, currentPage } = await pagination.getTablePagination()
